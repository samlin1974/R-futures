...
##
K<-3
mtxData2$dk<-(mtxData2$Kno+K-1)%/%K
mtxData2$dk1<-(mtxData2$Kno+K-1)%%K
#####
stockData <- mtxData2 %>%
  filter(flag=="A") 
################
...
inSiteTable <- stockData %>%
  filter(
    ...
    select(inDatetime=datetime,inKno=Kno, buyPrice=close,inNo=no)
...
MA05outSiteTable <- stockData %>%
  ...
  filter(
    ...
    lagClose1>MA05) %>%
  # 紀錄收盤價跌破05日移動平均線的發生日及當日收盤價，並重新命名欄位
  select(outDatetime=datetime,outKno=Kno, sellPrice=close,outNo=no,lagClose1,MA05)
...
MA20outSiteTable <- stockData %>%
  ...
  select(outDatetime=datetime,outKno=Kno, sellPrice=close,outNo=no,lagClose1,MA20)
...
# 目前的進場日期
  inNo <- inSiteTable$inNo[ix] 
# 找尋進場日期往後最近的出場位置
  outSite <- which(MA05outSiteTable$outNo>inNo)[1] 
...
for(ix in 1:nrow(inSiteTable)){
  ...
  inNo <- inSiteTable$inNo[ix] 
  # 找尋進場日期往後最近的出場位置
  outSite <- which(MA20outSiteTable$outNo>inNo)[1] 
  # 防呆機制，如果進場日期在資料尾端，有可能發生資料不足找不到出場位置的狀況
  if(length(outSite)>0){                            
    # 將該筆進場資訊與對應的出場資訊合併，並儲存至交易明細表內
...
# 計算持有日數
    holdTimes=as.numeric(outNo-inNo))
...
# 平均持有日數
avgHoldTimes <- mean(tradeDetailTable$holdTimes)   
##############
...
##PlotGraph <- function(plotSample){
  PlotGraph <- function(MA,plotSample){
      ...
      # 繪製交易樣本的進出場日期
        inDate <- tradeDetailTable$inDatetime[plotSample]
        outDate <- tradeDetailTable$outDatetime[plotSample] 
      # 繪圖起始日(進場日前35個交易日)，此處用ifelse避免繪製資料超出邊界
      matchSite <- which(stockData$datetime==inDate)-20
      plotStartDate <- stockData$datetime[ifelse(matchSite<1, 1, matchSite)]                           
      # 繪圖結束日(出場日後35個交易日)，此處用ifelse避免繪製資料超出邊界
      matchSite <- which(stockData$datetime==outDate)+20
      plotEndDate <- stockData$datetime[ifelse(matchSite>nrow(stockData), nrow(stockData), matchSite)]
      # 整理繪製的股價資料期間範圍及欄位資料
      stockData1 <-stockData %>%select(c(datetime,open:volume, MA5:MA60))
      plotData <- stockData1[which((stockData1$datetime>=plotStartDate)&(stockData1$datetime<=plotEndDate)),]
      # 取出繪圖資料所需的欄位
      plotData <- plotData %>% select(datetime:volume, MA5:MA60)
      ...
      # 繪製各交易日的K棒圖形(主圖)
      pic <- chart_Series(x=plotData[,1:5], name=paste0("台指小"," 技術分析圖形"), theme=myTheme)
      # 加入成交量圖形
      pic <- add_Vo()
      #指標
      #pic<-add_MACD()
...
# date        open  high   low close  volume lag